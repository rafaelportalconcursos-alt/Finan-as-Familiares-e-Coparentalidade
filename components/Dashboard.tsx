...
export const Dashboard: React.FC<DashboardProps> = ({ transactions, goals, pensionStatus, nextVisit, onDeleteGoal }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="glass p-10 rounded-[2.5rem] shadow-xl shadow-slate-100/30 dark:shadow-none flex flex-col min-h-[450px]">
          ...
          {/* Fix: Usando min-h e height fixo no wrapper para garantir que ResponsiveContainer tenha espaço */}
            ...
            {categoryData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                <PieChart>
                  ...
                  <Tooltip 
                    contentStyle={{ borderRadius: '24px', border: 'none', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.15)', padding: '16px', backgroundColor: isDark ? '#1e293b' : '#fff', color: isDark ? '#fff' : '#000' }}
                    itemStyle={{ fontWeight: 'bold', fontSize: '12px' }}
                  />
                </PieChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-full flex flex-col items-center justify-center text-slate-300 dark:text-slate-700 font-bold uppercase tracking-widest text-xs gap-4">
                 <div className="w-16 h-16 border-4 border-dashed border-current rounded-full"></div>
        ...
        <div className="glass p-10 rounded-[2.5rem] shadow-xl shadow-slate-100/30 dark:shadow-none overflow-y-auto max-h-[450px]">
          ...
          <div className="space-y-8">
            {goals.length === 0 && (
               <div className="py-10 text-center text-slate-400 font-bold italic text-sm">Nenhuma meta ativa no momento.</div>
            )}
            {goals.map(goal => (
              <div key={goal.id} className="group relative">
                <div className="flex justify-between items-end mb-3">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-slate-400 dark:text-slate-500 text-xs font-black">{Math.round((goal.currentAmount / goal.targetAmount) * 100)}%</span>
                    <button onClick={() => onDeleteGoal(goal.id)} className="text-slate-300 hover:text-rose-500 transition-all p-1 hover:scale-125" title="Excluir"><DeleteIcon /></button>
                  </div>
                </div>
                <div className="w-full bg-slate-100/50 dark:bg-slate-800/50 rounded-full h-4 p-1 border border-white dark:border-slate-700 shadow-inner transition-all">
                  <div className="bg-gradient-to-r from-indigo-500 to-indigo-400 h-full rounded-full transition-all duration-1000 shadow-md shadow-indigo-100 dark:shadow-none" style={{ width: `${Math.min(100, (goal.currentAmount / goal.targetAmount) * 100)}%` }}></div>
                </div>
