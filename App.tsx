...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initLoad = async () => {
      ...
      if (local) {
        try { setState(JSON.parse(local)); } catch (e) { console.error(e); }
      }
      try {
...
const FabSubButton = ({ onClick, label, icon, color }: any) => (
  <div className="flex items-center gap-3">
    ...
    <button onClick={onClick} className={`w-11 h-11 rounded-full flex items-center justify-center text-white shadow-xl hover:scale-110 transition ${color}`}>{icon}</button>
  </div>
);
